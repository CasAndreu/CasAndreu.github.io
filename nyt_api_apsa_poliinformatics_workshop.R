...
# To use the New York Times API you need first to register on the NYT 
#     website and then get an API Key. Go to the following web-address
#     (http://developer.nytimes.com/apps/register), click on 'Register'
#     (upper-right corner), enter your email and create a password.
#     Go back to the initial web address, log in, go to the APIs tab.
#	Click to '1.Request an API key'.
#     Enter a 'fake' temporary name for your application (e.g. Test).
#     Select the box 'Issue a new key for Article Search API'.
#	Scroll down to the bottom, agree to the terms, and select 
#     'Register Application'. You have created your key can find it
#	    in 'Keys' location of your NYT developer profile. 
...
# 3) SCRAPING ARTICLES (here, those mentioning 'Donald Trump' or 'Ted 
# Cruz' during Aug. 2015)

# Enter your key

options(nytimes_as_key = "#####################################") 

# Create some objects that will be used to store the results

trump <- NULL
cruz <- NULL

# Search for articles about Trump
...
# Search for articles about Cruz
...
# 3) PLOTTING NUMBER OF NEWS STORIES BY DAY
...
# 4) THE MOST USED ADJECTIVES IN TRUMP'S AND CRUZ'S NEWS STORIES?
...
# 5) ORDERING THE RESULTS AND OBTAINING RELATIVE FREQUENCIES
